...
/**
 * Shared project card (media + hover + arrow). Used by marquee and archive grid.
 *
 * @var WP_Post $postItem
 * @var bool $is_preview
 * @var string|null $imageSize
 * @var string|null $imageSizesAttr
 */
...
$resolvedImageSizesAttr = !empty($imageSizesAttr) ? $imageSizesAttr : null;
$ariaLabel = sprintf(
...
if ($resolvedImageSizesAttr !== null) {
    $imageAttrs['sizes'] = $resolvedImageSizesAttr;
    $hoverImageAttrs['sizes'] = $resolvedImageSizesAttr;
}
?>
       ...
       aria-label="<?php echo esc_attr($ariaLabel); ?>">
        ...
        <figure class="project-marquee-slider__media">
            ...
            <?php if ($hasImageHover && $hoverImageId > 0): ?>
                <?php echo wp_get_attachment_image($hoverImageId, $resolvedImageSize, false, $hoverImageAttrs); ?>
            <?php endif; ?>
        </figure>
