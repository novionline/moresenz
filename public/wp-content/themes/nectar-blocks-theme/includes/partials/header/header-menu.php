...
// Off canvas menu toggles point to the element they control ("simple" style toggles #mobile-menu instead).
$nectar_ocm_toggle_href = ( 'simple' === $nectar_header_options['side_widget_class'] ) ? '#mobile-menu' : '#slide-out-widget-area';
        ...
        <div class="row">
            <div class="col span_3">
                ...
                if( has_action('nectar_hook_mobile_header_before_logo') ) {
                    ?><span class="nectar-mobile-only"><?php nectar_hook_mobile_header_before_logo(); ?></span><?php
                }
                ...
                if ( $nectar_header_format === 'centered-menu-bottom-bar' ) {
                    ...
                    ?><nav class="left-side" data-using-pull-menu="<?php echo esc_attr( $has_pull_left_menu ); ?>">
                        ...
                        if( ! empty( $nectar_options['enable_social_in_header'] ) && $nectar_options['enable_social_in_header'] === '1' ) {
                            ?><ul class="nectar-social"><li id="social-in-menu" class="button_social_group"><?php nectar_header_social_icons( 'main-nav' ); ?> </li></ul><?php
                        }
                        ?>
                    </nav>
                    <nav class="right-side">
                        ...
                        if( true === $alt_button_location ) {
                            ?><ul class="buttons" data-user-set-ocm="<?php echo esc_attr( $nectar_header_options['user_set_side_widget_area'] ); ?>"><?php nectar_header_button_items(); ?></ul><?php
                        }
                        ...
                        if ( $nectar_header_options['side_widget_area'] === '1' || $nectar_header_options['side_widget_class'] === 'simple' ) {
                            ?><div class="slide-out-widget-area-toggle mobile-icon <?php echo esc_attr( $nectar_header_options['side_widget_class'] ); ?>" data-custom-color="<?php echo esc_attr($nectar_header_options['ocm_menu_btn_color']); ?>" data-icon-animation="simple-transform">
                                <div> <a href="<?php echo esc_attr( $nectar_ocm_toggle_href ); ?>" role="button" aria-label="<?php echo esc_attr__('Navigation Menu', 'nectar-blocks-theme'); ?>" aria-expanded="false" class="<?php echo 'closed' . esc_attr($menu_label_class); ?>">
                                    <?php if( true === $menu_label ) {
                                        echo '<i class="label">' . esc_html__('Menu', 'nectar-blocks-theme') . '</i>';
                                    } else {
                                        echo '<span class="screen-reader-text">' . esc_html__('Menu', 'nectar-blocks-theme') . '</span>';
            ...
            <div class="col span_9 col_last">
                ...
                if ( $nectar_header_options['header_search'] != 'false' ) {
                    ?><a class="mobile-search" href="#search-outer" role="button"><span class="nectar-icon icon-nectar-blocks-search" aria-hidden="true"></span><span class="screen-reader-text"><?php echo esc_html__('search', 'nectar-blocks-theme'); ?></span></a><?php
                }

                if ( $nectar_header_options['user_account_btn'] != 'false' && class_exists( 'WooCommerce' ) && function_exists('wc_get_page_id') ) {
                    ?><a class="mobile-user-account" href="<?php echo get_permalink( wc_get_page_id( 'myaccount' ) ); ?>"><span class="normal icon-nectar-blocks-m-user" aria-hidden="true"></span><span class="screen-reader-text"><?php echo esc_html__('account', 'nectar-blocks-theme'); ?></span></a><?php
                }
                ...
                if ( ! empty( $nectar_options['enable-cart'] ) && $nectar_options['enable-cart'] === '1' ) {
                    if ( $woocommerce ) {
                        ...
                        ?><a id="mobile-cart-link" data-cart-style="<?php echo esc_attr($nav_cart_style); ?>" role="button" href="<?php echo esc_url( wc_get_cart_url() ); ?>"><i class="icon-nectar-blocks-cart"></i><div class="cart-wrap"><span><?php echo esc_html( $woocommerce->cart->cart_contents_count ); ?> </span></div></a><?php
                    }
                }
                ...
                <nav>
                    ...
                    if( in_array($nectar_header_format, ['centered-logo-between-menu-alt'] ) ) {
                    ...
                    if ( false === in_array($nectar_header_format, ['menu-left-aligned','centered-logo-between-menu-alt'] ) &&
            ...
            <?php if ( in_array($nectar_header_format, ['menu-left-aligned','centered-logo-between-menu-alt'] ) ) { ?>
