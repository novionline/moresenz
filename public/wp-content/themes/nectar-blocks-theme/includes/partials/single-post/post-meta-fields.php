...
if ( have_posts() ) :
  while ( have_posts() ) :
    ...
    $meta_label_class = ('large' !== $author_style) ? ' nectar-font-label' : '';
    ...
    if( '1' !== $remove_post_date ) {
      ...
      if( 'last_edited_date' === $date_functionality ) {
        ...
        if( $nectar_u_modified_time >= $nectar_u_time + 86400 ) {
          $date_markup = '<span class="meta-date date published' . $meta_label_class . '">' . get_the_date() . '</span>';
          $date_markup .= '<span class="meta-date date updated rich-snippet-hidden' . $meta_label_class . '">' . get_the_modified_time(__( 'F jS, Y', 'nectar-blocks-theme' )) . '</span>';
        } else {
          $date_markup = '<span class="meta-date date updated' . $meta_label_class . '">' . get_the_date() . '</span>';
        }
      }
    }
    ...
    if( '1' !== $remove_reading_dur ) {
      $est_time_markup = '<span class="meta-reading-time' . $meta_label_class . '">' . nectar_estimated_reading_time(get_the_content()) . ' ' . esc_html__('min read', 'nectar-blocks-theme') . '</span>';
    }
    ...
    if( '1' !== $remove_post_author ) {
      ...
      echo '<span>';
      echo '<span class="fn nectar-font-label">';
      get_template_part('includes/partials/single-post/post-author');
      echo '</span>';
      if( 'large' === $author_style ) {
        $separator = '';
        if ( $date_markup && $est_time_markup ) {
          $separator = '<span>·</span>';
        }
        echo ' ' . '<span class="meta-secondary">' . $date_markup . $separator . $est_time_markup . '</span>';
      }
      echo '</span></span>';
    }
