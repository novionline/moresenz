...
if( in_array($header_format, ['default','centered-menu','menu-left-aligned']) ) {
  add_action('nectar_hook_before_logo', 'nectar_left_ocm_button_markup');
}
else if( in_array($header_format, ['centered-logo-between-menu-alt']) ) {
  add_action('nectar_hook_before_pull_left_items', 'nectar_left_ocm_button_markup');
  add_action('nectar_hook_before_logo', 'nectar_left_ocm_button_markup');
}
else if( in_array($header_format, ['centered-menu-bottom-bar']) ) {
  ...
  if( in_array($bottom_bar_align, ['left','left_t_center_b']) ) {
    add_action('nectar_hook_before_logo', 'nectar_left_ocm_button_markup');
  } else {
    add_action('nectar_hook_before_pull_left_items', 'nectar_left_ocm_button_markup');
  }
...
add_action('wp', 'nectar_ocm_fullscreen_split_markup_mods');
function nectar_ocm_fullscreen_split_markup_mods() {
    $nectar_options = get_nectar_theme_options();
...
function nectar_skip_to_content_link() {
    echo '<a href="#nectar-content-wrap" class="nectar-skip-to-content">' . esc_html__('Skip to main content', 'nectar-blocks-theme') . '</a>';
}
...
function nectar_mobile_header_persistent_items() {
  ...
  if( in_array($nectar_header_format, ['centered-logo-between-menu-alt','centered-menu-bottom-bar'] ) ) {
    $menu_locations[] = 'top_nav_pull_left';
  }

  if( in_array($nectar_header_format, [
    'menu-left-aligned',
