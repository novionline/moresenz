<?php
/**
 * NectarBlocks blog related functions
 *
        ...
        if( (is_category() || is_tag() || is_date() || is_author()) && isset(NectarThemeManager::$options['blog_archive_bg_functionality']) &&
            NectarThemeManager::$options['blog_archive_bg_functionality'] === 'color' ) {
                ...
                if( 'gradient' === $color_layout ) {
                    $styles['background'] = 'linear-gradient(180deg, ' . esc_attr($bg_color) . ' 0%, var(--nectar-overall-bg-color) 100%)';
                } else {
                ...
                if( is_category()) {
                    ...
                    if($cat_bg_color) {
                        ...
                        if( 'gradient' === $color_layout ) {
                            $styles['background'] = 'linear-gradient(180deg, ' . esc_attr($cat_bg_color) . ' 0%, var(--nectar-overall-bg-color) 100%)';
                        } else {
...
function nectar_blog_social_sharing() {
    // Output moved to "NectarBlocks Social" plugin.
}
 ...
 */
 if( ! function_exists('nectar_get_related_post_title') ) {
    ...
    function nectar_get_related_post_title() {
        global $nectar_options;
        $related_title_text = esc_html__( 'Related Posts', 'nectar-blocks-theme' );
        ...
        switch ( $related_post_title_option ) {
            ...
            case 'similar_posts':
                    $related_title_text = esc_html__( 'Similar Posts', 'nectar-blocks-theme' );
                    break;

            case 'you_may_also_like':
                    $related_title_text = esc_html__( 'You May Also Like', 'nectar-blocks-theme' );
                    break;
            case 'recommended_for_you':
                $related_title_text = esc_html__( 'Recommended For You', 'nectar-blocks-theme' );
                break;
            case 'hidden':
     ...
     function nectar_related_post_display() {
         ...
         if ( $current_categories ) {
             ...
             if( ! has_action('nectar_blog_loop_post_item') ) {
                ...
                echo '<div class="nectar-blocks__post-section full-width-section related-post-wrap" data-using-post-pagination="' . esc_attr( $using_post_pag ) . '" data-midnight="dark"> <div class="row-bg-wrap"><div class="row-bg"></div></div>';
                if( $related_post_title_option !== 'hidden' ) {
                    echo '<h3 class="' . $related_title_class . '">' . wp_kses_post( $related_title_text ) . '</h3>';
                }
                echo '<div class="row span_12 blog-recent related-posts columns-' . esc_attr( $related_post_count ) . '" data-style="' . esc_attr( $related_post_style ) . '" data-color-scheme="light">';
                            ...
                            <div <?php post_class( 'inner-wrap' ); ?>>
                                ...
                                if ( has_post_thumbnail() ) {
                                    ...
                                    if( true === $global_lazy_load ) {
                                        ...
                                        echo get_the_post_thumbnail( $post->ID, $related_image_size, [ 'title' => '' ] );
                                    }
                ...
                $related_title_class = apply_filters('nectar_related_posts_title_class', 'related-title');
                echo '<div class="vc_row-fluid full-width-section related-post-wrap post-loop-builder" data-midnight="dark">';
    ...
    if ( ! function_exists( 'nectar_blog_single_section_titles' ) ) {
        function nectar_blog_single_section_titles( $classname ) {
            ...
            if ( 'default' !== $blog_single_section_titles ) {
                ...
                // modify typography.
                return $classname . ' nectar-blog-single-section-title nectar-font-' . $blog_single_section_titles;
            }
