<?php
/**
 * NectarBlocks WooCommerce Integration.
 *
...
// Needed to let WooCommerce know NectarBlocks has theme options for columns
...
if ( ! function_exists( 'nectar_shop_header_output' ) ) {
    function nectar_shop_header_output() {
        global $post;
        $using_bg = nectar_using_page_header( $post->ID );
        $has_title = woocommerce_page_title(false) ? 'true' : 'false';
        echo '<div class="nectar-shop-header" data-using-bg="' . esc_attr($using_bg) . '" data-has-title="' . $has_title . '">';
...
if ( ! function_exists( 'nectar_shop_wrapper_start' ) ) {
    function nectar_shop_wrapper_start() {
        echo '<div id="nectar-content-wrap" class="container-wrap" data-midnight="dark">';
...
// Modify Breadcrumbs.
function nectar_woocommerce_page_header_title() {
 ...
 * the NectarBlocks specific structure
 *
 * @since 5.0
 */
    ...
    function nectar_woo_shop_markup() {
        ...
        if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
            ...
            if( 'contained' === $product_archive_header_size ) {
                add_action( 'nectar_shop_after_container_wrap_open', 'nectar_shop_header', 10 );
            }
            else {
                add_action( 'woocommerce_before_main_content', 'nectar_shop_header', 10 );
            }

            $product_archive_description = 'in_header';

            if ( ! function_exists( 'nectar_shop_header' ) ) {
                function nectar_shop_header() {
                    global $woocommerce;
            ...
            if ( ! function_exists( 'nectar_woo_shop_title' ) ) {
                function nectar_woo_shop_title() {
            ...
            $header_bg_image = (isset($nectar_options['product_archive_bg_image']) && isset($nectar_options['product_archive_bg_image']['url'])) ? $nectar_options['product_archive_bg_image']['url'] : '';

            $using_cat_bg = false;

            if ( is_shop() ) {
                if ( empty( $header_bg_image ) ) {
                    add_action( 'nectar_shop_header_markup', 'nectar_woo_shop_title', 10 );
                }
            }
            ...
            if( ! empty( $header_bg_color ) || ! empty( $header_bg_image ) || true === $using_cat_bg ) {
                ...
                add_action( 'nectar_shop_header_markup', 'woocommerce_breadcrumb', 9 );
                remove_action('woocommerce_archive_description', 'woocommerce_taxonomy_archive_description');
                remove_action('woocommerce_archive_description', 'woocommerce_product_archive_description');
                add_action('nectar_shop_header_markup', 'woocommerce_taxonomy_archive_description', 15);
            }

            // Filter sidebar toggle
            $product_filter_trigger = ( isset( $nectar_options['product_filter_area']) && '1' === $nectar_options['product_filter_area'] ) ? true : false;
            $main_shop_layout = ( isset( $nectar_options['main_shop_layout'] ) ) ? $nectar_options['main_shop_layout'] : 'no-sidebar';
            ...
            add_filter('nectar_before_page_header_title', 'nectar_woocommerce_page_header_title');

            if( true === $product_filter_trigger ) {

                add_action( 'nectar_shop_header_markup', 'nectar_shop_header_bottom_markup_output', 20 );
                ...
                add_action( 'nectar_shop_header_bottom_secondary_markup', 'nectar_shop_ordering_render', 10 );
...
if ( ! function_exists('nectar_shop_ordering_render') ) {
    function nectar_shop_ordering_render() {
        echo '<div class="nectar-font-label">';
...
if( ! function_exists('nectar_shop_header_alignment_class_mod') ) {
    function nectar_shop_header_alignment_class_mod( $classes ) {
        ...
        $cate = get_queried_object();
        $t_id = ( $cate && property_exists($cate, 'term_id') ) ? $cate->term_id : '';
        $product_terms = get_option( "taxonomy_$t_id" );
        ...
        echo '<div class="nectar-shop-filters">
					<a href="#" class="nectar-shop-filter-trigger nectar-font-label">
						...
						<span class="text-wrap">
						 ' . esc_html__('Filters', 'nectar-blocks-theme') . '</span>
					</a>';
...
if( ! function_exists('nectar_shop_sidebar_filter_area_meta') ) {
    function nectar_shop_sidebar_filter_area_meta() {
        echo '<div class="header">
			<span class="nectar-font-h5">' . esc_html__('Filters', 'nectar-blocks-theme') . '</span>
			<div class="nectar-close-btn-wrap">';
            echo '<a href="#" role="button" class="nectar-close-btn small">
				<span class="screen-reader-text">' . esc_html__('Close Filters', 'nectar-blocks-theme') . '</span>
...
if ( ! function_exists( 'nectar_override_pagination_args' ) ) {
    function nectar_override_pagination_args( $args ) {
        $args['prev_text'] = __( 'Previous', 'nectar-blocks-theme' );
        $args['next_text'] = __( 'Next', 'nectar-blocks-theme' );
        return $args;
    }
}

// move pagination location
if ( apply_filters('nectar_centered_woocommerce_pagination', true) ) {
    remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
    if ( class_exists( 'WooCommerce' ) ) {
        add_action( 'nectar_hook_before_container_wrap_close', 'woocommerce_pagination', 10 );
    }
}
    ...
    function woocommerce_output_related_products() {
        ...
        if( $content ) {
            ...
            if( '1' === $related_carousel ) {
                ...
                echo '<div class="span_12 dark"><div class="woocommerce"><div class="nectar-woo-carousel__top"><div class="swiper-pagination"></div></div><div class="nectar-woo-carousel" data-autorotate="" data-controls="arrows-overlaid">';
                echo $output;
                echo '</div></div></div>';
            }
            else {
...
if ( ! function_exists( 'nectar_woocommerce_header_add_to_cart_fragment' ) ) {
    function nectar_woocommerce_header_add_to_cart_fragment( $fragments ) {
        ...
        ob_start(); ?>
        <a class="cart-contents" aria-label="<?php echo esc_html__('Cart', 'nectar-blocks-theme'); ?>" href="<?php echo wc_get_cart_url(); ?>"><div class="cart-icon-wrap"><i class="icon-nectar-blocks-cart"></i> <div class="cart-wrap"><span><?php echo esc_html( $woocommerce->cart->cart_contents_count ); ?> </span></div> </div></a>
        <?php
...
if ( ! function_exists( 'nectar_mobile_woocommerce_header_add_to_cart_fragment' ) ) {
    function nectar_mobile_woocommerce_header_add_to_cart_fragment( $fragments ) {
        ...
        ?>
        <a id="mobile-cart-link" data-cart-style="<?php echo esc_attr($nav_cart_style); ?>" href="<?php echo wc_get_cart_url(); ?>"><i class="icon-nectar-blocks-cart"></i><div class="cart-wrap"><span><?php echo esc_html( $woocommerce->cart->cart_contents_count ); ?> </span></div></a>
        <?php
...
// Product styles.
add_action( 'wp', 'nectar_woo_product_styles' );

function nectar_woo_product_styles() {
    $nectar_options = get_nectar_theme_options();
            ...
            <a href="<?php the_permalink(); ?>"><?php
                            ...
                            if ( $product_hover_alt_image == '1' ) {
                                ...
                                if ( isset( $product_attach_ids[0] ) ) {
                                    $product_second_image = wp_get_attachment_image( $product_attach_ids[0], 'shop_catalog', false, [ 'class' => 'hover-gallery-image' ] );
                                }
                            }
                            ...
                            ?></a>
            <?php
                    echo '<div class="product-add-to-cart" data-nectar-quickview="' . esc_attr($nectar_quick_view) . '">';
       ...
       <div class="product-wrap">
            ...
            if( 'price_visible_flex_buttons' === $product_minimal_hover_layout ) {
				...
				<div class="product-image-wrap' . $product_second_image_class . '">
                    ...
                    echo woocommerce_get_product_thumbnail() . $product_second_image;
                echo '</a>
					<div class="product-add-to-cart" data-nectar-quickview="' . esc_attr($nectar_quick_view) . '">';
                    do_action( 'nectar_woocommerce_before_shop_loop_item_add_to_cart' );
                    woocommerce_template_loop_add_to_cart();
                    do_action( 'nectar_woocommerce_before_add_to_cart' );
                echo '</div></div>';
        ...
        if ( $woocommerce ) {
            ...
            <div class="cart-outer" data-user-set-ocm="<?php echo esc_attr( $userSetSideWidgetArea ); ?>" data-cart-style="<?php echo esc_attr( $nav_cart_style ); ?>">
                <div class="cart-menu-wrap">
                    <div class="cart-menu">
                        <a class="cart-contents" href="<?php echo wc_get_cart_url(); ?>"><div class="cart-icon-wrap"><i class="icon-nectar-blocks-cart" aria-hidden="true"></i> <div class="cart-wrap"><span><?php echo esc_html( $woocommerce->cart->cart_contents_count ); ?> </span></div> </div></a>
                    </div>
                </div>

                <?php if( 'slide_in_click' !== $nav_cart_style ) { ?>
                    <div class="cart-notification">
                        <span class="item-name"></span> <?php echo esc_html__( 'was successfully added to your cart.', 'nectar-blocks-theme' ); ?>
                    </div>
                <?php } ?>
...
if( ! function_exists('nectar_single_product_price_class') ) {
    function nectar_single_product_price_class($class) {
        ...
        return $class . ' nectar-font-' . esc_attr($inherit_typography);
    }
}
...
if( ! function_exists('nectar_single_product_review_with_average') ) {
    function nectar_single_product_review_with_average($title) {
      ...
      if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) {
              $nectar_title .= '<a class="nectar-button large regular accent-color regular-button nectar-product-reviews-trigger" href="#" data-color-override="false" data-hover-color-override="false"><span>' . esc_html__('Write a Review', 'nectar-blocks-theme') . '</span></a>';
      }
    ...
    function nectar_single_product_review_empty($title) {
        ...
        if( wc_review_ratings_enabled() && 'off_canvas' === $woo_review_style ) {
            ...
            if( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) {
                ...
                $nectar_title .= '<a class="nectar-button large regular accent-color regular-button nectar-product-reviews-trigger" href="#" data-color-override="false" data-hover-color-override="false"><span>' . esc_html__('Write a Review', 'nectar-blocks-theme') . '</span></a></div>';
...
if ( ! function_exists('nectar_activate_gutenberg_product') ) {
    function nectar_activate_gutenberg_product( $can_edit, $post_type ) {
...
add_filter( 'use_block_editor_for_post_type', 'nectar_activate_gutenberg_product', 10, 2 );
...
if( class_exists('YITH_WCAN_Frontend_Premium') ) {
    add_filter('nectar_woocommerce_sidebar_toggles', 'nectar_remove_woocommerce_sidebar_toggles');
}

if( ! function_exists('nectar_remove_woocommerce_sidebar_toggles') ) {
    function nectar_remove_woocommerce_sidebar_toggles() {
        return false;
    }
}
