/**
 * NectarBlocks upload/remove media.
 *
...
jQuery(document).ready(function($) {
  ...
  $(".nectar-add-btn").on('click',function(e) {
    ...
    image_add_frame.on( "select", function() {
      ...
      if( storeID ) {
        if ($('.' + $that.attr('rel-id') + '-url' ).length > 0 ) {
          $('.' + $that.attr('rel-id') + '-url' ).val(image_attachment.attributes.url).trigger('change');
        }

        if ( $('.' + $that.attr('rel-id') + '-id' ).length > 0 ) {
          $('.' + $that.attr('rel-id') + '-id' ).val(image_attachment.attributes.id).trigger('change');
        }

        // For items that were prev saved as a single value, still update it for backwards compatibility.
        if ( $('#' + $that.attr('rel-id') ).length > 0 ) {
  ...
  $(".nectar-remove-btn").on('click',function(e) {
    ...
    if( storeID ) {
      if ( $('.' + $(this).attr('rel-id') + '-url' ).length > 0) {
        $('.' + $(this).attr('rel-id') + '-url' ).val('');
      }
      if ( $('.' + $(this).attr('rel-id') + '-id' ).length > 0 ) {
        $('.' + $(this).attr('rel-id') + '-id' ).val('');
      }

      if ($('#' + $(this).attr('rel-id')).length > 0) {
