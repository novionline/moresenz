...
function nectar_autocomplete_init() {
    ...
    wp_register_script( 'my_acsearch', get_template_directory_uri() . '/nectar/assets/functions/ajax-search/wpss-search-suggest.js', [ 'jquery', 'jquery-ui-autocomplete' ], null, true );
    wp_localize_script( 'my_acsearch', 'MyAcSearch', [ 'url' => esc_url( admin_url( 'admin-ajax.php' ) ) ] );
...
function nectar_autocomplete_suggestions() {
    ...
    foreach ( $posts as $post ) :
        ...
        $suggestion['image'] = ( has_post_thumbnail( $post->ID ) ) ? get_the_post_thumbnail( $post->ID, 'thumbnail', [ 'title' => '' ] ) : '<i class="icon-nectar-blocks-pencil"></i>';
