...
if($product_style === 'material') {
    ...
    echo apply_filters(
        ...
        sprintf(
            ...
            esc_attr( isset( $args['class'] ) ? $args['class'] : 'button' ),
            isset( $args['attributes'] ) ? wc_implode_html_attributes( $args['attributes'] ) : '',
            '<span class="nectar-text-reveal-button"><span class="nectar-text-reveal-button__text" data-text="' . esc_attr($product->add_to_cart_text()) . '">' . esc_html( $product->add_to_cart_text() ) . '</span></span>'
        ),
        $product,
        $args
    );
